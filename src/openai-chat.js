...
export async function resolveCatalogMatch({
  ...
  const systemPrompt = [
    ...
    "",
    "- type product_detail_from_list: le message precedent du robot (visible dans l'historique de conversation) a propose une liste de plusieurs produits, et le client demande maintenant plus d'informations sur l'un d'entre eux (par exemple donne-moi plus d'infos sur le deuxieme, ou en me citant son nom, ou en citant son numero). Utilise lastProposedProducts pour identifier lequel des produits recemment proposes est vise, et retourne son identifiant dans productId. IMPORTANT: chaque produit affiche a l'ecran du client porte un badge #id correspondant exactement a son champ id dans lastProposedProducts (par exemple le produit avec id 5 affiche #5). Si le client designe un produit par un numero (le produit numero 5, le 5, l'article 5), ce numero fait reference a ce badge id affiche, PAS a la position du produit dans la liste ou dans lastProposedProducts: tu dois donc chercher dans lastProposedProducts l'element dont le champ id est exactement egal a ce numero, et ne jamais te baser sur le rang ou l'ordre. Ce n'est que si le client utilise un ordinal explicite en langage naturel (le premier, le deuxieme, le troisieme, the second one, etc, sans mentionner de numero affiche) qu'il faut alors se baser sur la position dans lastProposedProducts. Pour ce type, mets wantsDescription a true (le client demande explicitement plus d'informations). Ne mets wantsPrice a true que si le client demande aussi explicitement le prix.",
    "",
